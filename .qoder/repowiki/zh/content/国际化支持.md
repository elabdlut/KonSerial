...
**图表来源**
- [src/stores/settings.ts:81-97](file://src/stores/settings.ts#L81-L97)
- [src/stores/i18n.ts:314-347](file://src/stores/i18n.ts#L314-L347)
...
- 串口页与图表页在状态提示、按钮文案、帮助信息等场景统一使用翻译函数。
...
## 结论
KonSerial 的国际化方案以轻量、直观为核心：前端通过 i18n 存储模块与设置模块协同实现语言切换与文本渲染，后端通过 Tauri 命令与 JSON 文件完成配置持久化。该方案具备良好的可维护性与扩展性，适合在保持简洁的同时逐步引入更复杂的本地化策略。
...
**章节来源**
- [src/stores/i18n.ts:7-307](file://src/stores/i18n.ts#L7-L307)
...
**章节来源**
- [src/stores/i18n.ts:309-312](file://src/stores/i18n.ts#L309-L312)
...
- 示例三：图表页导出提示
  - 中文：数据已导出、图表已保存为 PNG
  - 英文：Data exported、Chart saved as PNG
...
- [src/stores/i18n.ts:158-307](file://src/stores/i18n.ts#L158-L307)